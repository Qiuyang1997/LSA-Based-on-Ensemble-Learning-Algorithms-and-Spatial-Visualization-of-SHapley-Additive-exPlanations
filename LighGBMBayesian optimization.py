...
param_space = {
    ...
    'min_gain_to_split': Real(0.001, 0.1, prior='log-uniform')
}

# 5. 使用Bayesian Optimization进行参数搜索和交叉验证

lgbm = lgb.LGBMClassifier()
bayes_search = BayesSearchCV(lgbm, param_space, cv=10, scoring='roc_auc', n_jobs=None, n_iter=50)
...
plt.grid(False)
plt.savefig(r'XXXXLGB.tif', dpi=500)
plt.show()

# 11. 保存交叉验证结果
results_df.to_excel(r'XXX\LGB.xlsx', index=False)
